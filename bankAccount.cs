...
public class BankAccount
{
    ...
    public bool Deposit(decimal amount)
    {
        ...
        _balance += amount;
        Console.WriteLine($"Success: Deposited {amount:C}. New balance is {_balance:C}.");
        return true;
    }
    ...
    public bool Withdraw(decimal amount, string inputPin)
    {
        ...
        if (inputPin != _pin)
        {
            ...
            {
                IsLocked = true;
                Console.WriteLine("Error: Invalid PIN code. Account has been LOCKED for security reasons after 3 failed attempts.");
            }
            else
            {
                Console.WriteLine($"Error: Invalid PIN code. (Attempts { _failedAttempts}/3)");
            }
            return false;
        }
        _failedAttempts = 0;
